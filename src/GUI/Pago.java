...
public class Pago extends javax.swing.JFrame {
    ...
    public Pago() {
     ...
     public Pago(DefaultTableModel mod,DefaultTableModel cant) {
            setUndecorated(true);
            initComponents();
            this.setSize(1086, 726);
            this.setLocationRelativeTo(null);
    ...
    private void btnAtrasActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAtrasActionPerformed
        ...
        this.setVisible(false);
        MenuInicio m = new MenuInicio(mod,cant);
        m.setVisible(true);
        }
    ...
    private void btnRegistroVentaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnRegistroVentaActionPerformed
        ...
        if((Float.parseFloat(lblTotalFinal.getText())<=0)&&(Float.parseFloat(lblTotal.getText()))>0){
           ...
           if((banderaE>0)&&(banderaC==0)){
            ...
            Ventas.executeQuery(Conexion.getDBConexion(), String.format("insert into ventas(usuario_id, total, venta_id, hora , fecha, metodo_clave) values(2, %s, %s ,(select to_char(sysdate,'hh24:mi:ss') from dual),(select to_date(sysdate,'dd/mm/yyyy') from dual), '%s')",total,1,metod));
            ...
            Ventas.executeQuery(Conexion.getDBConexion(), String.format("insert into ventas(usuario_id, total, venta_id, hora , fecha, metodo_clave) values(2, %s, %s ,(select to_char(sysdate,'hh24:mi:ss') from dual),(select to_date(sysdate,'dd/mm/yyyy') from dual), '%s')",total,Integer.parseInt(ven.getMax().toString())+1,metod));
